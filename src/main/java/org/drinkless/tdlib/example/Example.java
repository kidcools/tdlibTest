...
public final class Example {
    ...
    private static void getCommand() {
        ...
        String[] commands = command.split(" ", 10);
        try {
            switch (commands[0]) {
                case "gcs": {
                    int limit = 100;
                ...
                case "me":
                    ...
                    break;
                case "info":
                    System.out.println("查询群组:"+commands[1]);
                    client.send(new TdApi.GetChatHistory(Long.valueOf(commands[1]), Long.valueOf(commands[2]), Integer.valueOf(commands[3]), Integer.valueOf(commands[4]), true),defaultHandler);
                    break;
                case "download":
                    System.out.println("下载视频:"+commands[1]);
                    client.send(new TdApi.DownloadFile(Integer.valueOf(commands[1]), 1, 0, 0, true),defaultHandler);
                    break;
                case "upload":
                    System.out.println("上传视频:"+commands[1]+" 视频URL:"+commands[2]);
                    // 视频文件路径
                    //String videoPath = "/path/to/your/video.mp4";
                    TdApi.InputMessageVideo videoMessage = new TdApi.InputMessageVideo(new TdApi.InputFileLocal(commands[2]), null, null, 0, 0, 0, true, null, 0, false);
                    sendVideoMessage(Long.valueOf(commands[1]),videoMessage);
                    break;
                case "uploadRemote":
                    System.out.println(":"+commands[1]+" 视频URL:"+commands[2]);
                    // 视频文件路径
                    //String videoPath = "/path/to/your/video.mp4";
                    TdApi.InputMessageVideo videoMessage2 = new TdApi.InputMessageVideo(new TdApi.InputFileRemote(commands[2]), null, null, 0, 0, 0, true, null, 0, false);
                    sendVideoMessage(Long.valueOf(commands[1]),videoMessage2);
                    break;
                case "sm": {
    ...
    private static void sendVideoMessage(long chatId, TdApi.InputMessageVideo videoMessage) {
        // initialize reply markup just for testing
        TdApi.InlineKeyboardButton[] row = {new TdApi.InlineKeyboardButton("https://telegram.org?1", new TdApi.InlineKeyboardButtonTypeUrl()), new TdApi.InlineKeyboardButton("https://telegram.org?2", new TdApi.InlineKeyboardButtonTypeUrl()), new TdApi.InlineKeyboardButton("https://telegram.org?3", new TdApi.InlineKeyboardButtonTypeUrl())};
        TdApi.ReplyMarkup replyMarkup = new TdApi.ReplyMarkupInlineKeyboard(new TdApi.InlineKeyboardButton[][]{row, row, row});
        TdApi.MessageSendOptions messageSendOptions = new TdApi.MessageSendOptions();
        messageSendOptions.disableNotification  =true;

        client.send(new TdApi.SendMessage(chatId, 0, null, messageSendOptions, replyMarkup, videoMessage),  new Client.ResultHandler(){
            @Override
            public void onResult(TdApi.Object object) {

               if(object.getConstructor()==TdApi.UpdateFile.CONSTRUCTOR) {
                   TdApi.UpdateFile file = (TdApi.UpdateFile) object;
                   if(file.file.remote.isUploadingActive){
                       System.out.println("正在上传中FileDownload:" + file.file.local.path);
                   }else if(file.file.remote.isUploadingCompleted){
                       System.out.println("上传完成FileDownload:" + file.file.local.path);
                   }
...
//            @Override
//            public void onResult(TdApi.Object object) {
//                System.out.println("接受到消息了:"+object.toString());
//            }
//        }, null, null);
        // test Client.execute
        ...
        public void onResult(TdApi.Object object) {
            ...
            switch (object.getConstructor()) {
                case TdApi.File.CONSTRUCTOR:
                    ...
                    System.out.println(updateFile.file.local.path);
                    break;
                case TdApi.Messages.CONSTRUCTOR:
                    TdApi.Messages income = (TdApi.Messages) object;
                    TdApi.Message[] messages = income.messages;
                    Arrays.asList(messages).forEach(item->{
                        TdApi.MessageSenderUser senderId = (TdApi.MessageSenderUser) item.senderId;
                        Long userId = senderId.userId;
                    if (item.content instanceof TdApi.MessageVideo) {
                    TdApi.MessageVideo video = (TdApi.MessageVideo) item.content;
                    // 获取视频的缩略图信息
                    String thumbnail = video.video.thumbnail.toString();
                    int id = video.video.video.id;
                    // 在这里处理视频缩略图和JSON信息
                    //System.out.println("Video Thumbnail: " + thumbnail);
                    System.out.println("userID:"+userId+" "+item.id+":income video id:"+id);
                    System.out.println("income video :"+item.toString());
                    }else if(item.content instanceof TdApi.MessagePhoto){
                            TdApi.PhotoSize[] sizes = ((TdApi.MessagePhoto) item.content).photo.sizes;
                            System.out.println("userID:"+userId+" "+item.id+":income photo : " + sizes[sizes.length-1].photo.id);
                    }else if(item.content instanceof TdApi.MessageText){
                        System.out.println("userID:"+userId+" "+item.id+":income text: " + getMessageText(item));
                    }
                    });
                    return;
            }
            print(object.getConstructor()+"");
        }
    }

    private static class UpdateHandler implements Client.ResultHandler {

        private TdApi.MessageContent content;
...
//                        System.out.println("income text: " + message);
//                    }
                    //if(message.chatId==-975052697){
                        if(message.chatId==1613088925){
                        System.out.println("UpdateNewMessage:"+getMessageText(message));
                        //检查是否是视频消息
                        if (message.content instanceof TdApi.MessageVideo) {
                            ...
                            int id = video.video.video.id;
                            // 获取视频的JSON信息
                            TdApi.Object videoJson = video.video;
                            // 在这里处理视频缩略图和JSON信息
                            //System.out.println("Video Thumbnail: " + thumbnail);
                            System.out.println("income video: " + videoJson+"  id:"+id);
                            client.send(new TdApi.DownloadFile(Integer.valueOf(id), 1, 0, 0, true),defaultHandler);
                        }else if(message.content instanceof TdApi.MessagePhoto){
                            System.out.println("income photo : " + ((TdApi.MessagePhoto) message.content).photo);
                        }else if(message.content instanceof TdApi.MessageText){
                            System.out.println("income text: " + getMessageText(message));
                        }
                    }
                    if(message.isOutgoing && message.isChannelPost && message.isPinned){
                        System.out.println("收藏夹消息:"+getMessageText(message));
                        //检查是否是视频消息
                        if (message.content instanceof TdApi.MessageVideo) {
                            TdApi.MessageVideo video = (TdApi.MessageVideo) message.content;
                            // 获取视频的缩略图信息
                ...
                case TdApi.UpdateFile.CONSTRUCTOR:
                    ...
                    /*if(file.file.remote.isUploadingActive){
                        //System.out.println("正在上传中FileDownload:" + file.file.local.path);
                        long uploadedSize = file.file.remote.uploadedSize;
                        long downloadedSize = file.file.local.downloadedSize;

                        if(downloadedSize>uploadedSize) {
                            System.out.println("正在上传中File:上传进度" +decimalFormat.format((uploadedSize/downloadedSize)*100));
                        }else{
                            System.out.println("正下载中File:下载进度" +decimalFormat.format((downloadedSize/uploadedSize)*100));
                        }

                    }else if(file.file.remote.isUploadingCompleted){
                        System.out.println("上传完成FileDownload:" + file.file.local.path);
                    }else */if(file.file.local.isDownloadingActive){
//                        long uploadedSize = file.file.remote.uploadedSize;
//                        long downloadedSize = file.file.local.downloadedSize;
                        //System.out.println("正在下载中File:下载进度" +decimalFormat.format((downloadedSize/uploadedSize)*100));
                    System.out.println("正在下载中File:" +file.file.remote.id);
                    }else if(file.file.local.isDownloadingCompleted){
                        System.out.println("下载完成FileDownload:" + file.file.local.path);
                    }
                    break;
